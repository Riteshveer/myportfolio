...
const ProjectsSection = () => {
  ...
  return (
    <section id="projects" className="container" style={{ minHeight: '100vh', background: 'transparent', position: 'relative', padding: '60px 0 40px 0', overflow: 'hidden', fontFamily }}>
      <NeuralNetworkBackground />
      <h2 style={{ textAlign: 'center', fontSize: 'var(--h2-size)', fontWeight: 800, letterSpacing: 0.5, marginBottom: 8, color: accent, fontFamily }}>PORTFOLIO</h2>
      <div style={{ textAlign: 'center', color: '#666', fontSize: 'var(--caption-size)', marginBottom: 36, fontWeight: 500, fontFamily }}>
        A glimpse of the projects I've been working on
      </div>
      <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 44, flexWrap: 'wrap', fontFamily }}>
        {filters.map((f, i) => (
          <button
            key={f}
            onClick={() => setActive(i)}
      ...
      <div style={{
        ...
        {projects.map((proj, idx) => (
          <div key={proj.title} style={{
            ...
            <div>
              {proj.image && <div style={{ fontWeight: 700, fontSize: 'var(--header-size)', color: accent, marginBottom: 6, fontFamily }}>{proj.title}</div>}
              <div style={{ color: '#666', fontSize: 'var(--body-size)', marginBottom: 12, fontFamily }}>{proj.description}</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12, fontFamily }}>
                {proj.tags.map(tag => (
                  <span key={tag} style={{
                    background: '#f5f5f7',
                    color: accent,
              ...
              <div style={{ color: '#888', fontSize: 'var(--caption-size)', fontWeight: 500, fontFamily }}>Category: {proj.category}</div>
            </div>
          </div>
        ))}
      </div>
